...
1. `Server A functionality.md` — the original design document. **On any conflict,
   this document wins.**
2. `Design_A_Turnkey_Spec.md` (Revision 3) — the acceptance criteria (AC numbers).
3. `server_A_implementation_plan.md` (Revision 3) — scope, file layout, sequencing.

Four pillars every prompt serves: **Generalizability** (works with any MCU on any
toolchain — no test-specific hardcoding), **Simplicity** (Occam's razor — the most
efficient form that is still highly effective), **Correctness** (behavior matches
`Server A functionality.md` exactly), **Cleanliness** (neat, organized, easy for an
agent and a human to use — never a tangled web).
...
- Adversarial subagents are used only where a prompt says so, and their Codex
  side always uses model `gpt-5.6-sol` ("5.6 Sol", medium effort) — never
  5.6 Luna — since adversarial review is a distinct role from test execution.
  When an adversarial subagent reports findings, you must independently
  evaluate each finding for validity against `Server A functionality.md` before
...
## Prompt 1 — Orientation (read everything, build nothing)

PILLARS: correctness, cleanliness.

Read, in full: `Server A functionality.md`, `Design_A_Turnkey_Spec.md`,
...
build order (the plan's milestones M1–M7 and which prompts below map to them),
(b) any conflict you find between the three documents, resolved in favor of
`Server A functionality.md`, and (c) the exact list of files you will create,
taken from the plan.
...
## Prompt 2 — Brain skeleton: package, pair registration, locks, guides (plan M1)
...
pass. `Server A functionality.md` overrides all other documents.

Create the new subpackage `src/pyocd_debug_mcp/brain/` exactly as the plan's M1
describes:

- `brain/__init__.py` and `brain/server.py` — a wiring-only composition root: a
  stdio FastMCP server built on the existing `kernel/registry.RegistryFastMCP` /
  `ToolRegistry`. Register three Layer 2 agentic tools — `bug_fix`,
  `complex_implementation`, `complex_task` — locked by default, each with its own
  ...
  prompts replace it.
- `brain/session_state.py` — `BrainRun`: in-memory unlock set, a retained-context
  slot (filled in a later prompt), and an active-call latch field (armed in a
  later prompt). Nothing is ever persisted to disk.
- `brain/guides.py` — one guide per agentic tool containing: purpose, every
  parameter with what it must contain, context-quality expectations, **the three
  memory-construction prompts copied verbatim from `Server A functionality.md`**
  (single shared constants — identical across all three guides), and one complete
  example call. The memory prompts must appear nowhere except these guides — not
...
- `scripts/register_pair.py` behind a new `byo-pair-register` console script, and
  ...
  commands (Server B `--share <port>` — implemented in Prompt 5; Server A
  `--server-b-url ...`). No runtime discovery files.
- `tests/contracts/turnkey-brain-tools.json` — the new Server A contract file.
...
Satisfies AC-1.1, 1.2, 1.4, 2.1–2.6, 5.1, 5.2.
...
## Prompt 3 — Parameter contract and validation (plan M2, part 1)
...
- Typed models for every common parameter in the design doc's "Agentic tool input
  parameters" section: `tool_summary`, `task`, `memory_tier1_turn1`–`turn4` (each
  ...
  command, artifact paths), `iteration_max` (integer ≥ 1), `green_check_guide`,
  `green_check_script` (must state the exact command that runs it),
  `green_check_expected_outputs` (≥ 1 entry). Tool-specific: `bug`, `feature`,
  and `steps` (ordered list of non-empty strings — this is the design doc's
  `step_1 … step_n` carried as one list).
- **Opaqueness rule:** validate presence and shape only. Never parse, interpret,
  or branch on the content of board facts, build commands, paths, or artifacts —
...
- Tests: `tests/test_brain_params.py` — every missing common parameter named; a
...
## Prompt 4 — Prompt renderers, decision parser, workflows (plan M2, part 2)
...
- `brain/workflows.py` — the fixed step tables verbatim from the design doc:
  `bug_fix` = diagnose → locate root cause → patch → rebuild → flash → green
  check (6 steps); `complex_implementation` = understand requirement → implement
  → rebuild → flash → green check (5 steps); plus the `steps`-list source for
  `complex_task`.
- `brain/prompts.py` — `render_init_prompt(...)` producing the design doc's
...
- `brain/decisions.py` — the action index: exactly `next_step`, `continue_step`,
  ...
  fields, unknown actions, and missing/extra action parameters — each
...
## Prompt 5 — Server B shared endpoint: `--share <port>` (plan M3)

PILLARS: correctness (the design doc's "Server B Sharing" section: one shared
endpoint, one owner/queue of the board), simplicity (port fixed at registration —
...
- Add an opt-in `--share <port>` launch flag. When set, the same Server B process
  ...
  locks, plans, permissions, gates, and per-board serialization — zero behavior
...
- Tests: `tests/test_brain_serverb_share.py` — one process serves stdio and HTTP
  ...
  tool on one board serialize; `--share` absent ⇒ no listener; occupied port ⇒
  clear failure.
...
## Prompt 6 — Middleman session runtime (plan M4)

PILLARS: generalizability (any provider CLI via operator config — no vendor flags
...
- `brain/middleman_config.py` — an operator-owned JSON config loaded only from an
  ...
  config.
- `brain/middleman.py` — `MiddlemanSession` with exactly `start(init_prompt)`,
...
- A fake middleman provider executable under `tests/fixtures/` that replays a
  scripted list of decision replies (including malformed ones) — the workhorse
  for all later loop tests.
- Tests: `tests/test_brain_middleman.py` — fresh session per call with no state
...
## Prompt 7 — Turnkey loop engine (plan M5, part 1)
...
(validate → spawn middleman → run loop → return; returns are interim plain text
until Prompt 10):
...
- Step cursor is strictly monotonic — never revisits, never skips; `next_step`
  advances exactly one; `continue_step` re-issues the current step with a
  "please continue" injection.
- Iteration ledger: EVERY middleman decision — accepted, refused, or
  schema-rejected — consumes exactly one iteration (the design doc's own example
...
Satisfies AC-3.1–3.4, 7.1, 7.2, 7.4–7.7, 12.1–12.4 (sent halves), X.3, X.6.
...
## Prompt 8 — Green check (plan M5, part 2)

PILLARS: correctness (deterministic proof, never self-declared), generalizability
(Server A executes exactly the command the check declares — it never guesses an
...
- Materialize `green_check_guide` and `green_check_script` into the per-call
  ...
  exactly the manifest's entries — Prompt 10).
...
- `validate_green_check`: run the check by executing EXACTLY the command it
  ...
  `green_check_expected_outputs` is satisfied by the actual output — a literal,
...
## Prompt 9 — Adversarial checkpoint on the core (used sparingly)

PILLARS: all four.

RULES: you evaluate and fix yourself; the adversarial subagent only criticizes.
Its Codex side uses model `gpt-5.6-sol` ("5.6 Sol", medium effort) — the
adversarial-review model, distinct from the 5.6 Luna testing model.
`Server A functionality.md` overrides all other documents.

The core (parameters → prompts → decisions → loop → green check) is now the
...
against `Server A functionality.md` and the four pillars — find behavior that
...
`Server A functionality.md` and the spec before acting. Fix only the findings you
judge valid. Record a written disposition for each finding: valid → what you
changed; invalid → why. Do not launch a second adversarial run in this prompt.
...
## Prompt 10 — Returns and wrap-up cleanup (plan M6, part 1)

PILLARS: correctness (the two literal messages come from the design doc
word-for-word), cleanliness (manifest-only deletion — cleanup can never touch user
work).
...
- Every other non-success (fail_task, iteration exhaustion, middleman death or
  ...
  the middleman>; diagnose the issue and try again.` — substituting the
...
- `brain/cleanup.py`: from the loop's single exit finalizer, delete exactly the
  ...
  middleman plan/strategy artifacts, exchange files) — on success, fail,
...
- Tests: `tests/test_brain_returns_cleanup.py` — all four exit paths × cleanup;
...
Satisfies AC-9.1–9.4, 10.1–10.3, X.4.
...
## Prompt 11 — Delta (follow-up) calls (plan M6, part 2)
...
- After a completed full-form call, `BrainRun` retains the full context (memory
  ...
  iteration policy). In memory only — never on disk.
...
- This completes the permission round-trip: permission return → Client A obtains
  permission → delta "please continue" resumes without resupplying context.
- Tests: `tests/test_brain_delta.py` — retained context visible in the fake
...
## Prompt 12 — Cross-cutting hardening, docs, consolidated software gate (plan M7)

PILLARS: all four — this prompt closes the generalizability and cleanliness
...
- `tests/test_brain_import_closure.py`: `brain/` must import no `pyocd`, no
  board adapters, no Server B services, and no firmstore writers — making
...
- `tests/test_brain_hardening.py`: restart shows nothing persisted; a
  generalizability swap test — two entirely different fake board/toolchain
...
Satisfies AC-X.1, X.2, X.5, X.8–X.13; re-verifies AC-1.1–1.4 end-to-end.
...
## Prompt 13 — Bounded real-hardware smoke (both providers, real board)

PILLARS: correctness (proof on real hardware), generalizability (resolve the
board from live inventory — hardcode nothing).

RULES: code, read, and evaluate yourself; all test execution by subagents, once in
Codex `gpt-5.6-luna` (medium) and once in Claude `claude-sonnet-5` (medium); both must
pass or be honestly recorded as blocked. `Server A functionality.md` overrides
all other documents.

With the operator's explicit go-ahead and at least one board physically
connected (identify it through Server B's inventory — do not assume which board
it is):
...
4. Record evidence bundles (commands, MCP timelines, green-check output) under
   ...
   are unavailable, record blocked — never a pass.
...
TESTING: this entire prompt IS testing — performed only by the two provider
subagents connected to the servers. You orchestrate and evaluate; you do not
drive the boards yourself.

---

## Prompt 14 — FINAL TEST 1: adversarial refinement loop
...
`gpt-5.6-sol` ("5.6 Sol", medium effort) — the adversarial-review model, never
5.6 Luna. `Server A functionality.md` overrides all other documents.

Run this loop:

1. Launch an adversarial subagent (Codex `gpt-5.6-sol`, medium effort,
   read-only) to evaluate the ENTIRE product —
   Server A, the `--share` change, configs, docs, and tests — against the four
   ...
   behavior deviating from `Server A functionality.md`), and
...
2. For every finding, independently assess validity against
   `Server A functionality.md` and the spec. Implement fixes ONLY for findings
   you judge valid. Record a disposition for every finding (valid → fix made;
   invalid → reason).
...
TESTING: as embedded above — every re-verification is subagent-executed in both
Codex 5.6 Luna medium and Sonnet 5 medium.

---

## Prompt 15 — FINAL TEST 2: end-to-end product test from a blank repo

PILLARS: all four — this is the product's definition of done.
...
`Server A functionality.md` defines
...
5. **Then test Server B talking to Server A.** For each provider, run the full
   ...
   (permission return → grant → delta "please continue" → validated finish),
...
6. **Full project on real hardware.** Have each provider's subagent use the
   ...
   repo — one per connected board, using each board's own toolchain — via
...
DONE WHEN: both providers complete steps 4–7 with all green checks validated on
...
contains only the user's project work plus evidence — no leftover scratch, no
orphan processes, nothing persisted by Server A.

TESTING: this entire prompt IS the test — executed exclusively by the two
