...
2. Implement a Zephyr project detector/provider that runs one ordinary local `west build` through
   one coherently selected NCS environment with offline guards, OS-level descendant ownership,
   cwd-independent recovery markers, and no bootstrap behavior.
3. Return the exact parameterized general-helper argv/command and resolved local environment in
   setup status.
