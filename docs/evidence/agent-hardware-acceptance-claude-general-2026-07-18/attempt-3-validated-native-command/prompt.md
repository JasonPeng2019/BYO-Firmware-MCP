...
2. Reuse the Sonnet-authored application already present under `app-general/`; inspect it and require the dedicated thread, shell commands, and tagged transition prints. Do not rewrite it unless it is invalid. It must be a genuine Zephyr multithreaded program: a dedicated blink thread controls the board `led0` while the independent Zephyr UART shell accepts commands. Required commands and responses are `blink on` -> `BLINK ON`, `blink status` -> `BLINK STATUS: ON` or `BLINK STATUS: OFF`, and `blink off` -> `BLINK OFF`. Emit a tagged terminal print for each physical blink transition so UART evidence proves the blink thread executed. Use bounded sleeps, safe atomic/shared state, 115200 UART shell, and no networking, bootloader, MCUboot, security/provisioning, or erase behavior.

3. Build exactly once to the new `native-build-general-final/` directory using the validated local native NCS CLI below. This operator-supplied context was derived from the installed toolchain's own `C:\ncs\toolchains\936afb6332\environment.json`; it is the project's native CLI path, not the BYO Zephyr helper or fallback. Do not probe, configure, or build any other way and do not retry. You may write one `run-native-build-final.cmd` containing these environment values, then execute that script exactly once:
- prepend `C:\ncs\toolchains\936afb6332`, `mingw64\bin`, `bin`, `opt\bin`, `opt\bin\Scripts`, `opt\nanopb\generator-bin`, `nrfutil\bin`, `opt\zephyr-sdk\arm-zephyr-eabi\bin`, and `opt\zephyr-sdk\riscv64-zephyr-elf\bin` to PATH;
- set PYTHONPATH to the toolchain's `opt\bin`, `opt\bin\Lib`, and `opt\bin\Lib\site-packages`;
- set NRFUTIL_HOME to the toolchain's `nrfutil\home`, ZEPHYR_TOOLCHAIN_VARIANT=zephyr, and ZEPHYR_SDK_INSTALL_DIR to its `opt\zephyr-sdk`;
- change directory to `C:\ncs\v3.3.1` and execute exactly `C:\ncs\toolchains\936afb6332\opt\bin\python.exe -m west build -p always -b nrf52840dk/nrf52840 -s <absolute app-general> -d <absolute native-build-general-final> --no-sysbuild`.
A clean diagnostic already proved that exact command succeeds on this host; therefore any deviation or second build is a test failure. Record the exact command, environment selections, exit code, and local NCS/SDK paths. After success the coherent outputs are the native build's `zephyr\zephyr.elf`, `zephyr\zephyr.hex`, and `zephyr\zephyr.map`; confirm they exist and do not select any other image.

4. Call live MCP `collect_build_artifacts` once with those explicit ELF/HEX/map paths, `output_dir` set to new `collected-general-final/`, and `expected_roles` exactly `["elf", "hex", "map"]`. Use only its returned canonical paths afterward. Record its manifest and hashes. This is provenance only, not authority.
