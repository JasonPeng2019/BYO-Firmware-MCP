...
$env:PYTHONUTF8 = '1'
Set-Location $root
Get-Content (Join-Path $repo 'docs\evidence\wide-acceptance-claude-sonnet-5-medium-2026-07-18-prompt-r4.md') -Raw |
    claude -p --model claude-sonnet-5 --effort medium --permission-mode bypassPermissions --dangerously-skip-permissions --strict-mcp-config --mcp-config (Join-Path $repo 'docs\evidence\wide-acceptance-claude-sonnet-5-medium-2026-07-18-mcp-r4.json') --setting-sources local --no-session-persistence --output-format stream-json --verbose
