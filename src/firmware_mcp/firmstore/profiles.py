...
from firmware_mcp.firmstore.store import (
    FirmStore,
)
...
_PROVIDER_RECIPE_SUPPORT_FIELDS = frozenset(
    {"kind", "support_id", "provider_id", "target", "part_number"}
)

DeviceSupportVerifier = Callable[[str, BoardConfig, Mapping[str, str]], BoardConfig]
...
def _require_board_id(value: object) -> str:
    ...
    if not re.fullmatch(r"[a-z0-9_]+", board_id):
        raise ProfileError("board_id must contain lowercase letters, numbers, or underscores")
    return board_id
...
def _validate_device_support(value: object) -> dict[str, str] | None:
    ...
    expected_fields = (
        ...
        else _PROVIDER_RECIPE_SUPPORT_FIELDS
        if kind == "provider_recipe"
        else None
    )
    if expected_fields is None:
        raise ProfileError(
            "device_support.kind must be resolved_pack, resolved_builtin_target, or provider_recipe"
        )
    ...
    if kind != "provider_recipe" and re.fullmatch(r"[0-9a-f]{64}", result["support_id"]) is None:
    ...
    if kind != "provider_recipe" and re.fullmatch(r"[0-9a-f]{64}", result[digest_field]) is None:
...
def _verify_registered_device_support(
    ...
    *,
    ...
    if source.get("kind") == "provider_recipe":
        if store is None:
            raise ProfileError("provider recipe support replay requires the project recipe store")
        try:
            from firmware_mcp.firmstore.providers import ProviderRecipeStore

            recipe = ProviderRecipeStore(store).load(source["provider_id"])
            expected_support = recipe.support_identity(source["target"])
        except Exception as exc:  # noqa: BLE001 - recipe binding must replay exactly
            raise ProfileError(f"provider recipe support cannot be replayed: {exc}") from exc
        if source["support_id"] != expected_support:
            raise ProfileError("provider recipe support_id is not bound to provider_id and target")
        if (
            source["provider_id"] != board.provider_id
            or source["target"].casefold() != board.target.casefold()
        ):
            raise ProfileError("provider recipe support does not match the profile route")
        from firmware_mcp.setup_flow.device_support import normalize_part_number

        if normalize_part_number(source["part_number"]) != normalize_part_number(mcu_part_number):
            raise ProfileError("provider recipe support is bound to a different MCU part")
        return replace(board, provider_support_identity=source["support_id"])
    try:
    ...
    if expected["pyocd_target"].casefold() != board.target.casefold():
    ...
    if board.silicon_id_capability == "exact":
        if board.silicon_id_support_identity != expected["support_id"]:
            raise ProfileError(
                "exact profile identity evidence is not bound to replayed support_id"
            )
        if normalize_part_number(board.silicon_id_bound_part_number) != normalize_part_number(
            mcu_part_number
        ):
            raise ProfileError("exact profile identity evidence is bound to a different MCU part")
        if not board.silicon_id_provenance:
            raise ProfileError("exact profile identity evidence requires provenance")
        if actual_identity == (None, None, None):
            raise ProfileError(
                "exact profile identity evidence requires address, expected value, and mask"
            )
        # Exact part authority exists only when the replayed support itself
        # supplies the complete exact proof.  A caller-selected register may be
        # useful compatible evidence, but it cannot establish uniqueness for a
        # claimed part merely by matching a configured value.
        if proof is None or proof.capability != "exact":
            raise ProfileError(
                "profile exact identity evidence requires a replayed exact support proof"
            )
        if (
            board.silicon_id_addr != proof.address
            or board.silicon_id_expected != proof.expected
            or board.silicon_id_mask != proof.mask
            or board.silicon_id_width_bits != proof.width_bits
            or board.silicon_id_label != proof.label
            or board.silicon_id_provenance != proof.provenance
            or normalize_part_number(board.silicon_id_bound_part_number)
            != normalize_part_number(proof.bound_part_number)
            or board.silicon_id_support_identity != proof.support_identity
        ):
            raise ProfileError(
                "profile exact identity evidence does not match verified support proof"
            )
    elif proof is None:
        if actual_identity != (None, None, None):
            try:
    ...
    elif proof.capability != "compatible":
        raise ProfileError(
            "profile compatible identity evidence does not match an exact verified support proof"
        )
    elif (
    ...
    return replace(board, provider_support_identity=expected["support_id"])


def _validate_provider_live_identity(
    value: object, *, support: Mapping[str, str] | None, part: str
) -> None:
    """Retain typed provider identity without promoting compatible evidence."""

    if support is None or support.get("kind") != "provider_recipe":
        if value is not None:
            raise ProfileError("provider_live_identity is only valid for provider_recipe support")
        return
    required = {"capability", "part_number", "provenance", "support_identity", "evidence"}
    if not isinstance(value, Mapping) or set(value) != required:
        raise ProfileError("provider recipe profile requires complete provider_live_identity")
    if value["capability"] not in {"exact", "compatible", "unavailable"}:
        raise ProfileError("provider recipe live identity capability is invalid")
    if not isinstance(value["provenance"], str) or not value["provenance"].strip():
        raise ProfileError("provider recipe live identity requires provenance")
    if value["support_identity"] != support["support_id"]:
        raise ProfileError("provider recipe live identity is bound to a different support route")
    from firmware_mcp.setup_flow.device_support import normalize_part_number

    if value["capability"] == "exact":
        if not isinstance(value["part_number"], str) or not value["part_number"].strip():
            raise ProfileError("exact provider recipe live identity requires a part number")
        if normalize_part_number(value["part_number"]) != normalize_part_number(part):
            raise ProfileError("provider recipe live identity is bound to a different MCU part")
    elif value["part_number"] is not None:
        raise ProfileError("compatible/unavailable provider identity must not claim an exact part")
    try:
        import json

        json.dumps(value["evidence"], allow_nan=False)
    except (TypeError, ValueError) as exc:
        raise ProfileError("provider recipe live identity evidence is not JSON") from exc
    if value["capability"] == "unavailable":
        ...
        if (
            ...
            raise ProfileError("unavailable provider recipe live identity requires a reason")
...
def _normalize_legacy_schema_v2_document(document: dict[str, object], path: Path) -> bool:
    """Normalize the exact prior schema-v2 pyOCD dialect in memory only.

    Schema version 2 was retained when these fields moved, so they remain a
    supported on-disk dialect. Current mandatory live validation supersedes
    the old recovery-validation boolean; this compatibility step grants no
    recovery or target authority.
    """

    if document.get("schema_version") != PROFILE_SCHEMA_VERSION:
        return False

    # The original marker, not a current target field, is the whole dialect
    # boundary.  Current schema-v2 documents must continue through strict
    # validation untouched, including their unknown legacy-looking fields.
    if "pyocd_target" not in document:
        return False

    legacy_target = document["pyocd_target"]
    if not isinstance(legacy_target, str) or not legacy_target.strip():
        raise ProfileError(f"{path.name} legacy pyocd_target must be non-empty text")
    if "target" in document:
        target = document["target"]
        if not isinstance(target, str) or not target.strip() or target != legacy_target:
            raise ProfileError(
                f"{path.name} target and pyocd_target must be exact-equivalent non-empty text"
            )
    else:
        document["target"] = legacy_target
    document.pop("pyocd_target")

    if "provider_id" not in document:
        document["provider_id"] = "pyocd"

    if "requires_recover_validation" in document:
        if type(document["requires_recover_validation"]) is not bool:
            raise ProfileError(f"{path.name} legacy requires_recover_validation must be a boolean")
        document.pop("requires_recover_validation")

    if "recover_mode" in document:
        recover_mode = document["recover_mode"]
        if recover_mode is not None:
            if not isinstance(recover_mode, str):
                raise ProfileError(f"{path.name} legacy recover_mode must be text or null")
            normalized_mode = recover_mode.strip().casefold()
            if normalized_mode not in {"", "backend_mass_erase", "manual_only"}:
                raise ProfileError(
                    f"{path.name} legacy recover_mode is unsupported: {recover_mode!r}"
                )
        document.pop("recover_mode")

    return True


def _canonicalize_legacy_device_support(
    *, store: FirmStore, part_number: str, source: Mapping[str, str]
) -> dict[str, str]:
    """Convert only verified released support IDs to current authority IDs.

    The compatibility route is reached exclusively after the original
    ``pyocd_target`` marker identified a released schema-v2 document.  It
    replays every backing fact before comparing the historical ID, then emits
    the current closed authority record for normal validation.
    """

    try:
        from firmware_mcp.setup_flow.device_support import (
            _resolve_persisted_builtin_target_binding,
            _resolve_persisted_pack_binding,
        )

        if source["kind"] == "resolved_pack":
            candidate = _resolve_persisted_pack_binding(store, part_number, source)
        elif source["kind"] == "resolved_builtin_target":
            candidate = _resolve_persisted_builtin_target_binding(part_number, source)
        else:
            return dict(source)
        if source["support_id"] != candidate.legacy_schema_v2_support_id:
            raise ProfileError(
                "legacy device_support support_id does not match verified backing facts"
            )
        return candidate.to_authority_document()
    except ProfileError:
        raise
    except Exception as exc:  # noqa: BLE001 - profile support must fail closed
        raise ProfileError(
            f"legacy device_support cannot be replayed from verified support: {exc}"
        ) from exc
...
class ProfileRepository:
    ...
    def _from_v2_document(self, raw: Mapping[str, object], path: Path) -> BoardProfile:
        ...
        is_legacy_pyocd_dialect = _normalize_legacy_schema_v2_document(document, path)
        ...
        if is_legacy_pyocd_dialect and device_support is not None:
            device_support = _canonicalize_legacy_device_support(
                store=self.store,
                part_number=part_number,
                source=device_support,
            )
            document["device_support"] = device_support
        if (
            device_support is not None
            and device_support.get("kind") != "provider_recipe"
            and (datasheet_sha256 is None or datasheet_ref is None)
        ):
            raise ProfileError(
                "generic device_support requires captured datasheet_sha256 and datasheet_ref"
            )
        _validate_provider_live_identity(
            document.get("provider_live_identity"), support=device_support, part=part_number
        )
        ...
        if device_support is not None:
            board = self._device_support_verifier(part_number, board, device_support)
    ...
    def commit_core(self, staged: StagedProfile) -> BoardProfile:
        ...
        with (
            safety_publication_lock(self.store.layout.project_root, staged.profile.board_id),
            self._commit_lock,
        ):
    ...
    def restore_snapshot(self, board_id: str, previous: BoardProfile | None) -> None:
        """Restore the exact pre-setup profile after a paired recipe publication fails.

        This deliberately owns only the named profile path while holding the
        repository lock.  It is a setup transaction rollback, not a general
        profile deletion facility.
        """

        identity = _require_board_id(board_id)
        with safety_publication_lock(self.store.layout.project_root, identity), self._commit_lock:
            target = self.store.layout.board_profile(identity)
            if previous is None:
                if target.exists():
                    target.unlink()
                return
            if previous.board_id != identity:
                raise ProfileError("profile rollback snapshot belongs to another board")
            self.store.atomic_write_yaml(target, previous.to_document())
    ...
    def publish_safety_map(
        self, staged: StagedProfile, map_path: Path, map_bytes: bytes
    ) -> BoardProfile:
        """Publish the sole map and its profile association as one rollback bundle.

        The caller holds the per-board durable safety-publication lock; this
        method adds the repository freshness check and its normal in-process
        serialization without introducing another authority path.
        """
        if staged.operation != "safety_ref" or staged.expected_updated_at is None:
            raise ProfileError("publish_safety_map requires a staged safety reference")
        try:
            import yaml  # type: ignore[import-untyped]
        except ImportError as exc:  # pragma: no cover
            raise ProfileError("PyYAML is required to publish a safety map") from exc
        with (
            safety_publication_lock(self.store.layout.project_root, staged.profile.board_id),
            self._commit_lock,
        ):
            current = self.load(staged.profile.board_id)
            if current.updated_at != staged.expected_updated_at:
                raise StaleProfileStageError("Profile changed before safety-map publication")
            self._assert_display_available(staged.profile)
            profile_text = yaml.safe_dump(
                staged.profile.to_document(),
                allow_unicode=True,
                default_flow_style=False,
                sort_keys=False,
            )
            self.store.atomic_write_bundle(
                {
                    map_path: bytes(map_bytes),
                    self.store.layout.board_profile(staged.profile.board_id): profile_text.encode(
                        "utf-8"
                    ),
                }
            )
            return self._load_v2_path(self.store.layout.board_profile(staged.profile.board_id))
