...
class BoardConfig:
    ...
    silicon_id_capability: str = "compatible"
    silicon_id_provenance: str = ""
    silicon_id_bound_part_number: str = ""
    silicon_id_support_identity: str = ""
...
def make_board_config(raw: dict[str, object], source_path: Path | None) -> BoardConfig:
    ...
    required_fields = ["board_id", "display_name", "mcu_family", "probe_family", "target"]
    ...
    if not re.fullmatch(r"[a-z0-9_]+", board_id):
        raise ConfigError("board_id must contain lowercase letters, numbers, or underscores")
    ...
    provider_id = str(raw.get("provider_id") or "pyocd").strip()
    if not provider_id or "/" in provider_id or "\\" in provider_id or "\0" in provider_id:
        raise ConfigError("provider_id must be a non-empty separator-free string")
    ...
    probe_type = str(raw.get("probe_type") or probe_family).strip()
    ...
    default_terms = tokenize_hint_text(board_id, display_name, mcu_family, target)
    ...
    if silicon_fields_present:
        ...
        silicon_id_capability = str(raw.get("silicon_id_capability") or "compatible").strip()
        silicon_id_provenance = str(raw.get("silicon_id_provenance") or "").strip()
        silicon_id_bound_part_number = str(raw.get("silicon_id_bound_part_number") or "").strip()
        silicon_id_support_identity = str(raw.get("silicon_id_support_identity") or "").strip()
        if silicon_id_capability not in {"exact", "compatible"}:
            raise ConfigError("silicon_id_capability must be exact or compatible")
        if silicon_id_capability == "exact" and not all(
            (
                silicon_id_provenance,
                silicon_id_bound_part_number,
                silicon_id_support_identity,
            ...
            raise ConfigError(
                "exact silicon identity requires provenance, bound part number, and support identity"
            )
    ...
    return BoardConfig(
        ...
        silicon_id_capability=silicon_id_capability,
        silicon_id_provenance=silicon_id_provenance,
        silicon_id_bound_part_number=silicon_id_bound_part_number,
        silicon_id_support_identity=silicon_id_support_identity,
