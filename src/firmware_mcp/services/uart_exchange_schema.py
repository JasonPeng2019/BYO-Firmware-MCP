...
def validate_exchange_serial_parameters(parameters: Mapping[str, object]) -> str | None:
    """Return a precise refusal reason, or ``None`` for one executable exchange."""

    supplied = set(parameters)
    if supplied != SERIAL_EXCHANGE_FIELDS:
        return (
            "exchange_serial parameters must match exactly; "
    ...
    if not _finite_number_at_least(parameters["timeout_seconds"], minimum=0.000001):
        return "timeout_seconds must be a positive finite number"
    baud = parameters["baud"]
    if baud is not None and (isinstance(baud, bool) or not isinstance(baud, int) or baud <= 0):
        return "baud must be a positive integer or NULL"
    ...
    ready_seconds = parameters["ready_timeout_seconds"]
    ...
    if ready_text is None:
        if ready_seconds != 0 or probe_text is not None or probe_delay != 0:
            return (
                "without ready_text, ready_timeout_seconds and ready_probe_delay_seconds must be 0 "
    ...
    if not 0 < float(ready_seconds):
        return "ready_text requires positive ready_timeout_seconds"
    ...
    if float(probe_delay) > float(ready_seconds):
        return "ready_probe_delay_seconds must not exceed ready_timeout_seconds"
    return None
