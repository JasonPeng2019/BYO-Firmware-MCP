...
3. Fetch those PDSCs one-by-one.
...
# Stream buffering only; it does not cap a descriptor download or request duration.
_CHUNK = 1 << 16
...
def fetch_master_index(
    index_url: str = DEFAULT_MASTER_INDEX_URL, timeout: float | None = None
) -> list[PdscRef]:
    try:
...
def _download_descriptor(ref: PdscRef, dest: Path, *, timeout: float | None) -> None:
    ...
    try:
        with httpx.stream(
            ...
            timeout=timeout,
            headers={"User-Agent": "byo-firmware-mcp/pack-index-repair"},
        ) as response:
            response.raise_for_status()
        ...
        raise PackIndexRepairError(f"Failed to fetch {ref.remote_pdsc_url}: {exc}") from exc
...
def repair_live_pack_index(
    *,
    ...
    for ref in downloads:
        _download_descriptor(ref, descriptor_path(cache_dir, ref), timeout=timeout)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional caller-selected request timeout in seconds.",
    )
    parser.add_argument(
        "--index-url",
