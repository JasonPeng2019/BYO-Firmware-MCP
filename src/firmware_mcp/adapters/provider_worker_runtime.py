...
from firmware_mcp.adapters.swd_pyocd import (
    PyOCDSWDInterface,
)
from firmware_mcp.board_config import BoardConfig
from firmware_mcp.probe_inventory import probe_family_from_pyocd_probe
...
# Keep this in lock-step with the parent: v4 carries typed cleanup diagnostics.
_VERSION = 4
_BOARD_FIELDS = {field.name for field in fields(BoardConfig)}
_ERROR_KINDS: tuple[tuple[type[TargetControlError], str], ...] = (
    (TargetConnectionCleanupError, "target_connection_cleanup"),
...
def _validate_arguments(operation: str, arguments: dict[str, Any]) -> None:
    ...
    if operation == "recover":
        _exact(arguments, {"mechanism"})
        _text(arguments["mechanism"], "mechanism")
        return
...
def _error_record(error: Exception) -> dict[str, object]:
    """Serialize only the small typed error facts the parent can validate."""

    kind = _error_kind(error)
    record: dict[str, object] = {"kind": kind, "message": str(error)}
    if isinstance(error, TargetConnectionCleanupError):
        record["primary"] = {
            "type": error.primary_error_type,
            "message": error.primary_error_message,
        }
        record["cleanup_diagnostics"] = [item.to_record() for item in error.cleanup_diagnostics]
    return record
...
def _board(raw: object) -> BoardConfig | None:
    ...
    nullable_text = {"debug_protocol", "debug_connect_mode", "expected_uart_substring"}
...
def _metadata(handle: TargetSessionHandle) -> dict[str, Any]:
    ...
    return {
        ...
        # pyOCD's selected target is support metadata, not a hardware-read exact part.
        # Slice-1 silicon proof remains the direct pyOCD flash authority.
        "live_identity": {
            "capability": "compatible",
            "part_number": None,
            "provenance": "pyOCD live target session metadata",
            "support_identity": f"pyocd:{metadata.target_override or 'default'}",
            "evidence": {"target_override": metadata.target_override},
        },
    }
...
def _dispatch(
    ...
    if operation == "get_state":
        ...
        result = [region.to_record() for region in adapter.physical_memory_regions(handle)]
    ...
    elif operation == "recovery_capabilities":
        result = [item.to_record() for item in adapter.recovery_capabilities(handle)]
    elif operation == "recover":
        result = adapter.recover(handle, str(arguments["mechanism"])).to_record()
