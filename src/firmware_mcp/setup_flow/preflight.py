...
from firmware_mcp.firmstore.cache import CacheResolution
...
class SetupUserInput:
    ...
    def __post_init__(self) -> None:
        if not _BOARD_ID.fullmatch(self.board_id):
            raise PreflightError("board_id must contain lowercase letters, numbers, or underscores")
    ...
    def evaluate(
        ...
        if current.blocking_error is not None:
            return PreflightDecision(
                ...
                current.blocking_error.message,
                observed=observed,
            )
