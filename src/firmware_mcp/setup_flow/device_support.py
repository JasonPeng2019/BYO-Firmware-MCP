...
def _legacy_schema_v2_proof_document(
    proof: LiveIdentityProof | None,
) -> dict[str, object] | None:
    """Render the six fields used by the released schema-v2 support IDs.

    The previous on-disk dialect predates provenance/support bindings in
    ``LiveIdentityProof.to_document()``.  Keeping this conversion here makes
    its narrowly scoped historical digest reproducible without relaxing the
...
def live_cpuid_compatibility_proof(observed_cpuid: int) -> LiveIdentityProof:
    ...
    try:
        observed_cpuid = validate_identity_observation(observed_cpuid, 32)
...
class DeviceSupportCandidate:
    ...
    @property
    def legacy_schema_v2_support_id(self) -> str:
        """Return the exact pre-provider-neutral profile identifier.

        This is deliberately retained only for the profile repository's
        recognized ``pyocd_target`` schema-v2 dialect.  New authority records
        always use :attr:`support_id` and must never accept this value.
        """

        proof = _legacy_schema_v2_proof_document(self.identity_proof)
        material = "\0".join(
            (
                self.pack_sha256,
...
class BuiltInTargetSupportCandidate:
    ...
    @property
    def legacy_schema_v2_support_id(self) -> str:
        """Return the exact pre-provider-neutral built-in identifier.

        Only recognized legacy schema-v2 records may be checked against this
        digest during read-only normalization.
        """
        ...
        material = {
            ...
            "pyocd_target": self.pyocd_target.casefold(),
            "geometry_sha256": self.geometry_sha256,
            "identity_proof": _legacy_schema_v2_proof_document(self.identity_proof),
        }
        return hashlib.sha256(
            json.dumps(material, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
...
class PackAddressRegion:
    ...
    def readable(self) -> bool:
        return access_facts(self.access)[0]

    @property
    def writable(self) -> bool:
        return access_facts(self.access)[1]

    @property
    def executable(self) -> bool:
        """Whether this exact upstream access token explicitly permits execution."""

        return access_facts(self.access)[2]


# CMSIS-Pack/SVD access spellings are schema values, not free-form prose.  Keep
# this one exact vocabulary shared by support replay and safety-map
# reconciliation: a new spelling must be intentionally added with its complete
# R/W/X meaning instead of being guessed from a substring.
_ACCESS_FACTS: dict[str, tuple[bool, bool, bool]] = {
    "r": (True, False, False),
    "w": (False, True, False),
    "rx": (True, False, True),
    "rw": (True, True, False),
    ...
    "writeOnce": (False, True, False),
    "read-write": (True, True, False),
    "read-writeOnce": (True, True, False),
}


def access_facts(access: str) -> tuple[bool, bool, bool]:
    """Return exact readable/writable/executable support facts for ``access``."""

    try:
        return _ACCESS_FACTS[access]
    except (KeyError, TypeError) as exc:
        raise PackProvisionError(
            f"Unsupported exact support access token {access!r}; repair the support evidence."
        ) from exc


@dataclass(frozen=True, slots=True)
class SvdRegisterCapability:
    """Optional SVD-derived register metadata and its precise diagnostic state."""

    status: str
    regions: tuple[PackAddressRegion, ...] = ()
    reason: str | None = None
    error_type: str | None = None
    error_message: str | None = None

    def __post_init__(self) -> None:
        if self.status not in {"available", "unavailable", "error"}:
            raise PackProvisionError("SVD capability status is invalid")
        if self.status == "available" and (self.error_type or self.error_message):
            raise PackProvisionError("available SVD capability cannot contain an error")
        if self.status == "error" and (not self.error_type or not self.error_message):
            raise PackProvisionError("SVD capability error must include type and message")

    def to_document(self) -> dict[str, object]:
        return {
            "status": self.status,
            "regions": [item.to_document() for item in self.regions],
            "reason": self.reason,
...
class PackMemoryGeometry:
    ...
    svd_register_capability: SvdRegisterCapability = SvdRegisterCapability("unavailable")
    ...
    def to_document(self) -> dict[str, object]:
        return {
            ...
            "svd_register_capability": self.svd_register_capability.to_document(),
        }
...
def _resolve_persisted_builtin_target_binding(
    part_number: str, authority: Mapping[str, str]
) -> BuiltInTargetSupportCandidate:
    """Re-resolve complete built-in backing facts without trusting its ID."""
...
def _resolve_persisted_pack_binding(
    store: FirmStore,
    part_number: str,
    authority: Mapping[str, str],
) -> DeviceSupportCandidate:
    """Re-resolve complete pack backing facts without trusting its support ID.
    ...
    for manifest_path, packs_dir in sources:
        for spec in load_manifest(manifest_path):
            ...
            for binding in spec.device_bindings:
                ...
                if replayed.identity_proof != binding.identity_proof:
                    raise PackProvisionError(
                        "Persisted device binding identity proof no longer matches "
                        "the exact verified PDSC leaf"
                    )
                candidate = DeviceSupportCandidate.from_verified_pack(selected, binding)
...
) -> SvdRegisterCapability:
    """Parse optional SVD register metadata without silently losing failures."""

    try:
        stream = getattr(device, "svd", None)
        if stream is None:
            return SvdRegisterCapability("unavailable", reason="support source provides no SVD")
        payload = stream.read()
        if not isinstance(payload, bytes) or not payload:
            return SvdRegisterCapability(
                "unavailable", reason="support source provides no SVD payload"
            )
            ...
            if registers:
                for register in registers:
                    try:
                        ...
                        if width_bits <= 0:
                            raise ValueError("SVD register width must be positive")
                    ...
                    except (AttributeError, TypeError, ValueError, PackProvisionError) as exc:
                        return SvdRegisterCapability(
        ...
        return SvdRegisterCapability("available", _intersect_access_regions(rows))
    except Exception as exc:  # noqa: BLE001 - report optional parser failure exactly
        return SvdRegisterCapability("error", error_type=type(exc).__name__, error_message=str(exc))
...
def resolve_registered_pack_geometry(
    ...
    svd_capability = _svd_peripheral_regions(
        device,
    ...
    return PackMemoryGeometry(
        ...
        canonical_ram,
        canonical_rom,
        svd_capability.regions or canonical_peripherals,
        cpu_system_regions,
        svd_capability,
    )
