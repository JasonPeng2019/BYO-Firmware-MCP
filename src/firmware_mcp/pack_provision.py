...
# Stream buffering only; it does not cap a pack download or request duration.
_CHUNK = 1 << 16
...
class LiveIdentityProof:
    ...
    def __post_init__(self) -> None:
        ...
        # Assumption: only a cited source that says this masked value uniquely
        # distinguishes the bound exact part may call the evidence exact.
        if self.capability == "exact" and not all(
            isinstance(value, str) and value.strip()
            for value in (self.provenance, self.bound_part_number, self.support_identity)
        ):
            raise PackProvisionError(
                "exact identity proof requires provenance, bound_part_number, and support_identity"
            )
    ...
    def to_document(self) -> dict[str, object]:
        return {
            ...
            "bound_part_number": self.bound_part_number,
...
def _device_bindings(value: object) -> tuple[DeviceBinding, ...]:
    ...
    for index, item in enumerate(value):
        ...
        if raw_proof is not None:
            ...
            extended_proof_fields = proof_fields | {
                "provenance",
                "bound_part_number",
                "support_identity",
            }
            if not isinstance(raw_proof, dict) or (
                set(raw_proof) != proof_fields and set(raw_proof) != extended_proof_fields
            ):
            ...
            proof = LiveIdentityProof(
                ...
                str(raw_proof["label"]),
                str(raw_proof.get("provenance", "")),
                str(raw_proof.get("bound_part_number", "")),
                str(raw_proof.get("support_identity", "")),
            )
...
def _download(url: str, dest: Path) -> None:
    ...
    try:
        ...
        # The selected pack source owns its network timing.  A slow valid
        # download must not be rejected by a server-invented deadline.
        ...
        try:
            ...
            attach_cleanup_error(exc, f"pack staging removal (retained path: {tmp})", cleanup)
