...
- Pre-POSIX accepted-candidate hashes:
  `.change-loop/fresh-suite/H00/PRE_POSIX_REPAIR_MANIFEST.json`
...
- the POSIX repair adds no `type: ignore`, `# pyright` suppression, `Any`
  import/annotation/cast, or Pyright configuration change;
- no existing Windows cleanup behavior, failure identity, marker retention,
  deadline, or cleanup-bound contract regresses.

The neutral change-loop gate runs only the two isolated tester-recorded
commands. After that gate is green, root separately materializes one complete
candidate containing the accepted README/metadata/lock changes, the
`processes.py` repair, and both existing H00 tests. No additional tester or
production file is authorized, so the candidate overlay has one fixed
self-contained six-file set.
Root records a SHA-256 manifest, requires every staged Windows and POSIX file to
match it, and then runs the seven commands above in fresh native Windows and
Debian WSL2/ext4 candidates, including ordinary and space-plus-Unicode paths.
No host may receive a partial overlay.

After both root-owned host gates pass, the recorded H00 owner (transitioned to
one persistent `gpt-5.4-mini` session under the model-continuity contract)
receives those exact candidate bytes and reruns only the failed/touched H00
requirements. Previously verified requirements and raw evidence are preserved;
the model handoff does not restart H00.

## Ownership

- Doer production ownership:
  `src/pyocd_debug_mcp/kernel/processes.py` only. No second production helper
  is authorized. The doer must not edit tests or the previously accepted H00
  metadata/docs/lock changes.
- Spec tester owns its existing
  `tests/test_h00_repository_contract.py`; it may not add another file. Its
  existing test may change only for the named portability assertions and to
  make its nested clean candidate include the complete manifest-defined
  candidate instead of the current three-file overlay.
- Regression tester owns its existing
  `tests/test_h00_repository_regressions.py`; it may not add another file.
