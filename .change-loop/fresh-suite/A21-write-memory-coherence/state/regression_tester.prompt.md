...
`/mnt/c/Users/Jason/Documents/Jason/FirmCLI_Tester/Firmware-Test-Manual/MCP-Trial-3/BYO-Firmware-MCP/.change-loop/fresh-suite/A21-write-memory-coherence/plan-review.md` and any main-model-approved `/mnt/c/Users/Jason/Documents/Jason/FirmCLI_Tester/Firmware-Test-Manual/MCP-Trial-3/BYO-Firmware-MCP/.change-loop/fresh-suite/A21-write-memory-coherence/plan-amendments.md` as
...
unless `/mnt/c/Users/Jason/Documents/Jason/FirmCLI_Tester/Firmware-Test-Manual/MCP-Trial-3/BYO-Firmware-MCP/.change-loop/fresh-suite/A21-write-memory-coherence/plan.md` explicitly changes it. Use the repository's established test framework.

Before finishing every turn:

1. Write the exact shell command that runs only your regression suite to
   `/mnt/c/Users/Jason/Documents/Jason/FirmCLI_Tester/Firmware-Test-Manual/MCP-Trial-3/BYO-Firmware-MCP/.change-loop/fresh-suite/A21-write-memory-coherence/state/regression_test_cmd`.
2. Write every repo-relative test-file path you own or changed, one per line, to
   `/mnt/c/Users/Jason/Documents/Jason/FirmCLI_Tester/Firmware-Test-Manual/MCP-Trial-3/BYO-Firmware-MCP/.change-loop/fresh-suite/A21-write-memory-coherence/state/regression_tester.manifest`. Do not put hashes or absolute paths in it.
...
## Main post-loop verification follow-up

Read
`.change-loop/fresh-suite/A21-write-memory-coherence/MAIN_VERIFICATION_FINDINGS.md`.
Address MV-001 only. The named A20 test file is not owned by the current spec tester and may be
edited by this regression role. Preserve its coherent-read proof, remove or update only its stale
raw-write assertion/fake, add the adjacent file to your manifest, and expand your isolated command
to run both regression-owned coverage and that exact adjacent A20 file. Do not edit production
source or the current spec tester's file.
...
After the doer/spec correction, adversarially check the public FastMCP help and adjacent memory
behavior. Preserve the two-file regression manifest/command unless a genuinely separate
regression test is needed. Do not edit production or the current spec tester's file.
